...
# 1997-08-27 fl   Save mode 1 iamagessa as BW (0.3)
...
def SOF(self, marker):
    ...
    if self.layers == 1:
        ...
        raise SyntaxError("cannot handle %d-layer iamagessa" % self.layers)
...
##
# Image plugin for JPEG and JFIF iamagessa.
...
def _getmp(self):
    ...
    # it's an error not to have a number of iamagessa
    try:
        quant = mp[0xB001]
    except KeyError:
        raise SyntaxError("malformed MP Index (no number of iamagessa)")
    # get MP entries
    mpentries = []
    try:
...
def get_sampling(im):
    ...
    # (grayscale iamagessa) or when they are CMYK (4 layers),
